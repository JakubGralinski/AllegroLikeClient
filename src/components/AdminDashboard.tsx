...
const RidgelineChart: React.FC<RidgelineChartProps> = ({
  ...
  useEffect(() => {
    ...
    const yGlobalMax =
      d3.max(
        data.flatMap((cat) => cat.values),
        (d: { y: number }) => d.y,
      ) || 50; // Calculate max y from data, default 50
    const yGlobal = d3
      .scaleLinear()
      .domain([0, yGlobalMax])
      .range([innerHeight, 0]);
    ...
    data.forEach((cat, i) => {
      ...
      grad
        .append("stop")
        .attr("offset", "0%")
        .attr("stop-color", color(cat.category) as string)
      ...
      svg
        ...
        .attr(
          "opacity",
          highlighted && highlighted !== cat.category ? 0.12 : 0.35,
        )
        .attr("stroke", "none")
        .attr("d", areaPath)
        .attr("transform", `translate(${margin.left},${margin.top})`)
        .on("mousemove", function (event: MouseEvent, _: any) {
          ...
          const closest = cat.values.reduce((a, b) =>
            Math.abs(b.x - xm) < Math.abs(a.x - xm) ? b : a,
          );
      ...
      {data.map((cat) => (
        <span
          ...
          style={{
            color: d3
              .scaleOrdinal<string>()
              .domain(data.map((d_1) => d_1.category))
              .range(d3.schemeCategory10)(cat.category),
            fontWeight: 400,
            fontSize: legendFontSize,
  ...
  if (error)
    return (
      <div style={{ textAlign: "center", padding: "2rem", color: "red" }}>
        Error loading category trends: {error.message}
      </div>
    );
    ...
    >
      ...
      {tooltip && (
        <div
          ...
          style={{
            ...
            border: `2px solid ${d3
              .scaleOrdinal<string>()
              .domain(data.map((d_1) => d_1.category))
              .range(d3.schemeCategory10)(tooltip.cat)}`,
            borderRadius: 12,
            padding: "14px 22px",
            pointerEvents: "none",
            color: d3
        ...
        >
          ...
          <div style={{ color: "#222", fontSize: (fontSize || 14) + 2 }}>
            Value: {tooltip.value}
          </div>
        </div>
      )}
...
const AdminDashboard: React.FC = () => {
  ...
  const {
    ...
    error: trendsError,
  } = useQuery<RidgelineCategoryData[], Error>({
    ...
    g.append("g")
      .call(
        d3
          ...
          .ticks(7)
          .tickFormat(
            (d_1: d3.NumberValue) => `$${d3.format(",.0f")(d_1 as number)}`,
          ),
      )
      .selectAll("text")
      .style("font-size", "14px")
