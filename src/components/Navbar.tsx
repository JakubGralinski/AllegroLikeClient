...
function Navbar() {
    ...
    return (
        <>
            <div
                className={`${
                    isMobileOpen ? "z-100 w-[80%] left-[0.5]" : "max-md:hidden left-[2vw]"
            ...
            <button
                onClick={() => toggleIsMobileOpen()}
                className={`md:hidden fixed top-4 p-3 text-3xl text-white rounded-2xl ${isMobileOpen ? "bg-secondary" : "bg-primary"} transition-all duration-200 ease-in-out transform hover:scale-[1.02] hover:shadow-md cursor-pointer`}
            >
