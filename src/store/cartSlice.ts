...
const cartSlice = createSlice({
  ...
  reducers: {
    addItemToCart: (state, action: PayloadAction<Omit<CartItem, 'quantity'> & { quantity?: number }>) => {
      console.log('[cartSlice] addItemToCart action:', JSON.parse(JSON.stringify(action))); // Log the action
      console.log('[cartSlice] current cart state before add:', JSON.parse(JSON.stringify(state))); // Log current state
      ...
      console.log('[cartSlice] new cart state after add:', JSON.parse(JSON.stringify(state))); // Log new state
    },
    removeItemFromCart: (state, action: PayloadAction<number>) => { // Payload is itemId
